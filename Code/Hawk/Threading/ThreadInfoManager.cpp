...
namespace ThreadInfoManager
{
	...
	using LogTextMap = std::unordered_map<std::thread::id, std::string>;
	LogTextMap n_LogTextMap;
	Mutex n_LogTextMutex("ThreadLogText");

	using SysIDMap = std::unordered_map<std::string, std::thread::id>;
	SysIDMap n_SysIDMap;
...
void ThreadInfoManager::RegisterThread(const std::string& p_Name, std::thread::id p_SysThreadID, ThreadID p_ID)
{
	...
	{
		MutexScope l_MutexScope(n_LogTextMutex);
		THROW_IF_NOT(n_LogTextMap.insert(LogTextMap::value_type(p_SysThreadID, CreateLogText(p_Name, p_SysThreadID, p_ID))).second, "Failed to insert thread log text entry. Name=" << p_Name << " ID=" << p_ID);
	}
	{
		MutexScope l_MutexScope(n_SysIDMutex);
		THROW_IF_NOT(n_SysIDMap.insert(SysIDMap::value_type(p_Name, p_SysThreadID)).second, "Failed to insert system thread entry. Name=" << p_Name << " ID=" << p_ID);
	}
