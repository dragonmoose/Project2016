...
: m_RenderView(p_RenderView)
{
	...
	Microsoft::WRL::ComPtr<ID2D1Factory3> l_D2DFactory;
	THROW_IF_COMERR(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory3), &l_D2DFactoryOptions, &l_D2DFactory), "Failed to create D2D factory");
	...
	THROW_IF_COMERR(l_D2DFactory->CreateDevice(l_DXGIDevice.Get(), &m_D2DDevice), "Failed to create d2d device");
	...
	float l_fDpiX;
	float l_fDpiY;
	l_D2DFactory->GetDesktopDpi(&l_fDpiX, &l_fDpiY);
	...
	for (int i = 0; i < p_RenderView->GetBackBuffers().size(); i++)
	{
		THROW_IF_COMERR(m_D3D11On12Device->CreateWrappedResource(p_RenderView->GetBackBuffers()[i].Get(), &l_ResourceFlags, D3D12_RESOURCE_STATE_RENDER_TARGET,
